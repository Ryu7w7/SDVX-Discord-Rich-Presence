...
public:
  DiscordIPC(const std::string &cid)
      : hPipe(INVALID_HANDLE_VALUE), client_id(cid), connected(false) {}
  ...
  bool Connect() {
    ...
    hPipe =
        CreateFileA("\\\\.\\pipe\\discord-ipc-0", GENERIC_READ | GENERIC_WRITE,
    ...
    // Send handshake
    char payload[256];
    snprintf(payload, sizeof(payload), "{\"v\": 1, \"client_id\": \"%s\"}",
             client_id.c_str());
  ...
  void SetActivity(const std::string &state, const std::string &details,
                   ...
                   const std::string &small_text, long long start_time) {
    if (!Connect())
      return;
    ...
    if (start_time > 0) {
      len += snprintf(payload + len, sizeof(payload) - len,
                      "\"timestamps\": {\"start\": %lld}, ", start_time);
    }
    ...
    snprintf(payload + len, sizeof(payload) - len, "}}}, \"nonce\": \"1\"}");
